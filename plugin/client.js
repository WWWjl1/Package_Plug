/**
 * 芯片封装学习 · 插件 Client 半（界面）
 * ---------------------------------------------------------------------------
 * 这一份是「阶段一：动态 Cordis 插件」的 Client 半源码。
 * 用法：把它整份作为 cordis_define 的 code.client 传入。
 *
 * 这是一个函数体，不能用 import / require / TypeScript / JSX。
 * 可用的全局符号只有：ctx / React / host / styles / console（见 Builtin.listBuiltins）。
 *
 * 它往 dsh 里注册三样东西：
 *   1. sidebar.footer.action —— 左侧栏最下方的「📚 芯片封装学习」入口
 *   2. conversation.input.dock —— 只在学习会话出现的一条提示（进过一次学习界面后自动隐藏）
 *   3. conversation.view —— 学习界面本体（顶栏标签「📚 进入学习界面」）
 *
 * 学习界面布局：上＝工序按钮 + 知识树 + 正文/剖面图/小测；下＝dsh 原生输入框。
 * 在学习界面里发送消息后，插件会自动把视图切到 dsh 原生「对话」界面看答案。
 */
...
function EntryButton(props) {
  ...
  const onClick = () => {
    ...
    host.call('get-session', {}).then((saved) => {
      ...
      return host.call('ensure-workspace', {}).then((res) => {
        ...
        if (wsId) {
          return ui.connectWorkspace(wsId).then((id) => {
            if (id) {
              host.call('save-session', { sessionId: String(id) }).catch(() => {})
...
function LearnView(props) {
  ...
  React.useEffect(() => {
    ...
    host.call('set-status', { id: pointId, status: 'learning' }).catch(() => {})
  }, [allowed, pointId])
  ...
  const mark = (id, next) => {
    ...
    host.call('set-status', { id, status: next }).catch(() => {})
  }
...
return {
  ...
  apply(ctx) {
    ...
    if (slots === undefined) {
      console.error('package-learn: slots service unavailable')
      return
    }
    ...
    slots.inject('sidebar.footer.action', () => slots.register(
      ...
      (slotProps) => h(EntryButton, { wide: slotProps.wide === true, uiWorkspace, sessions }),
    ))
