...
>
> **要改代码先看 [`CONTEXT.md`](CONTEXT.md)**：那里有「想改哪儿看哪儿」的索引、已经定下来的设计决策
> （以及为什么）、踩过的坑、验收脚本能抓什么 / 抓不到什么。看那一份就不用通读整个工程。

---

## 界面长什么样

**整屏**（真实截图，在 dsh 里打开学习界面之后的样子）：

![学习界面](docs/ui-overview.png)

左边是知识树（三条主线 / 19 道工序 / 完成度），右边是正文——带表格、剖面图、重点框，
正文下面是「相关知识点」的跨模块跳转、小测和「问 AI」快捷提问；底部还是 dsh 原生输入框。

**顶栏三行**（所有入口都在这里）：

![顶栏](docs/ui-topbar.png)

| 行 | 内容 |
|---|---|
| 第一行 | 三条主线 + **错题重做**、**今日复习**（各自带数量，点进去是占满整块区域的独立界面） |
| 第二行 | 搜索（同时搜标题 / 正文 / 题目）+ **课程自检**、**备份与导出**、**学习统计**、**设置** |
| 第三行 | 当前主线的工序按钮 |

<details>
<summary>同一张图，纯文本版（方便搜索 / 复制）</summary>

```
┌──────────────────────────────────────────────────────────────┐
│ [传统封装流程][先进封装][专题模块]              ← 三条主线     │
│ [错题重做 3][今日复习 5]           ← 两个独立界面（占整块区域）│
│ [搜索…] [课程自检] [备份与导出] [学习统计] [设置]  ← 工具区     │
...
├─ tools/                     ← 验收脚本（不属于插件运行路径，删掉不影响插件）
│  └─ check-all.mjs           ← 一条命令跑完：node tools/check-all.mjs
...
- [x] **复习机制** —— 「今日复习」间隔重复队列（错题优先 / 在学 3 天 / 已掌握 1·3·7·16·35 天）
- [x] **备份与导出** —— 进度导出导入（覆盖前强制备份）+ 课程导出 Markdown（可打印）
- [x] **学习统计** —— 时长 / 连续天数（口径保守：不把「窗口挂了一夜」算成学习）
- [x] **验收脚本进仓库** —— `tools/`：内容 / 主机半 / 客户端半 / **真渲染一遍界面**
...
四个检查依次跑：课程内容 / 主机半 / 客户端半 / 界面渲染，任一不过就以非 0 退出。
渲染那一项会**在临时目录里把学习界面完整渲染一遍、再点几下按钮**，不会碰你真实的进度。
细节与「它抓不到什么」见 [`tools/README.md`](tools/README.md)。

> 说句实在的：**样式好不好看、有没有被挤扁，脚本看不出来**（它只能查「同名选择器重复定义」
> 这类静态问题）。改完 `lib/client.js` 的样式，一定要在浏览器里看一眼。
