...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function LearnView(props) {
      ...
      const saveExport = () => {
        if (!backup || typeof backup !== 'object' || !backup.url) return
        try {
      ...
      if (needle !== '') {
        ...
        const kids = [h('div', { className: 'pkl-check-head', key: 'h' }, '进度备份 · 导出 / 导入')]
        ...
        const acts = [
          h('button', {
            ...
            onClick: doExport,
          }, backup === 'loading' ? '正在导出…' : '导出为 JSON 文件'),
          h('button', {
            type: 'button',
            className: 'pkl-btn',
            key: 'i',
          ...
          ref: (el) => {
            ...
            h('div', { className: 'pkl-toolbar', key: 'tools' }, [
              ...
              h('button', {
                ...
                key: 'b',
                title: '把学习进度导出成一个 JSON 文件；也可以从文件导回来（覆盖前自动备份）',
