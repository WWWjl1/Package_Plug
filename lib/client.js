...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      '.pkl-entry-label{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
      '.pkl-chips-inline{display:flex;flex-wrap:wrap;gap:7px}',
      ...
      // flex:0 0 auto —— 按钮永不压缩：宁可换行，也不要被挤成一条缝或溢出被裁掉
      '.pkl-track{flex:0 0 auto;font:inherit;font-size:12px;padding:3px 11px;border-radius:999px;border:1px solid var(--dsw-alias-border-l1);background:transparent;color:var(--dsw-alias-label-secondary);cursor:pointer}',
      ...
      // 必须允许换行：这一行有搜索框 + 四颗按钮，窄窗口下不换行就会往右溢出，
      // 被外层容器裁掉之后按钮会"凭空消失"，用户根本不知道有这些功能。
      // （原来那个 margin-left:auto 在这里是无效的：父级是块级容器，不是 flex。）
      '.pkl-toolbar{display:flex;align-items:center;gap:6px;flex-wrap:wrap;justify-content:flex-end;margin-bottom:2px}',
      '.pkl-search{font:inherit;font-size:12.5px;width:210px;max-width:100%;padding:4px 10px;border-radius:999px;border:1px solid var(--dsw-alias-border-l1);background:transparent;color:var(--dsw-alias-label-primary)}',
      ...
      '.pkl-chart{display:flex;align-items:flex-end;gap:8px;height:74px;margin:6px 0 16px}',
      '.pkl-chart-col{display:flex;flex-direction:column;align-items:center;justify-content:flex-end;gap:5px;min-width:34px}',
      '.pkl-chart-bar{width:22px;min-height:2px;border-radius:4px 4px 0 0;background:var(--dsw-alias-brand-primary);opacity:.85}',
      '.pkl-chart-day{font-size:11px;color:var(--dsw-alias-label-secondary)}',
    ...
    function LearnView(props) {      const [data, setData] = React.useState(null)
      ...
      if (point === null) {
        ...
        mainKids.push(h('div', { className: 'pkl-askbar', key: 'ask' }, [
          ...
          h('div', { className: 'pkl-chips-inline', key: 'c' }, chips.map((chip, i) => h('button', {
            type: 'button',
            className: 'pkl-chip',
            key: i,
      ...
      if (needle !== '') {
        ...
        if (study === null || study === 'loading') {
          ...
          kids.push(h('div', { className: 'pkl-chart', key: 'bars' }, sum.bars.map((b) => h('div', {
            className: 'pkl-chart-col',
            key: b.key,
            title: b.key + ' · ' + humanTime(b.seconds),
          }, [
            h('div', { className: 'pkl-chart-bar', key: 'b', style: { height: (maxBar > 0 ? Math.round((b.seconds / maxBar) * 46) : 0) + 'px' } }),
            h('div', { className: 'pkl-chart-day', key: 'd' }, b.key.slice(5)),
          ]))))
