...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ------------------------------------------------------------------
    // B2：对话框是悬浮在内容之上的，而且会随「多行输入」变高。
    // 所以正文底部留白不能写死，必须实时量出来。
    // 做法：在对话框正上方埋一个零高度探针，量出输入区总高度，发布给学习界面。
    // ------------------------------------------------------------------
    const composerPad = { value: 214, listeners: [] }

    function publishPad(next) {
      const v = Math.max(160, Math.round(next))
      if (v === composerPad.value) return
      composerPad.value = v
    ...
    function LearnView(props) {
      ...
      // 错题重做：再点一次收起
      const toggleWrong = () => {
        if (panel === 'wrong') { setPanel(null); return }
        setPanel('wrong')
        setQuery('')
      }
      ...
      if (needle !== '') {
        ...
        const kids = [h('div', { className: 'pkl-check-head', key: 'h' }, '错题重做 · 共 ' + list.length + ' 道')]
        if (list.length === 0) {
          kids.push(h('div', { className: 'pkl-check-ok', key: 'ok' }, '✓ 错题本是空的'))
        } else {
          kids.push(h('div', { className: 'pkl-check-line', key: 'tip' }, '先选、可改，提交后才批改；答对了会自动从错题本移出。'))
            ...
            const footer = done
              ...
              : h('div', { className: 'pkl-submit-row', key: 'e' }, [
                ...
                h('button', {
                  ...
                  className: 'pkl-btn',
                  key: 'g',
                  onClick: () => jumpTo(item.processId, item.pointId),
                }, '去看这个知识点'),
              ])
            kids.push(h('div', { className: 'pkl-q', key: i }, [
              h('div', { className: 'pkl-q-text', key: 'q' }, item.title + ' · 第 ' + String(item.qi + 1) + ' 题：' + String(item.q.q)),
              h('div', { className: 'pkl-opts', key: 'o' }, opts),
              footer,
            ]))
          }
      ...
      return h('div', { className: 'pkl-view' }, [
        h('div', { className: 'pkl-learn', key: 'learn', style: { paddingBottom: pad + 'px' } }, [
          ...
          className: 'pkl-bar',
          key: 'bar',
          // B3：把自己真实的高度报上去，供知识树做吸附偏移
          ref: (el) => {
            if (!el) return
            const bar = Math.round(el.offsetHeight)
            let max = 560
            try {
              max = Math.max(200, Math.round(window.innerHeight - bar - 250))
            } catch (error) {
              max = 560
            }
            ...
            h('div', { className: 'pkl-toolbar', key: 'tools' }, [
              ...
              h('button', {
                type: 'button',
                className: 'pkl-track' + (panel === 'wrong' ? ' pkl-track-on' : ''),
                key: 'w',
                title: '把答错过的题集中起来重做，答对就自动移出错题本',
                onClick: toggleWrong,
              }, '错题重做'),
              h('button', {
                type: 'button',
