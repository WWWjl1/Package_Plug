...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function LearnView(props) {
      ...
      // 课程自检：再点一次收起
      const runSelfCheck = () => {
        if (check !== null) { setCheck(null); return }
        setCheck('loading')
      ...
      if (check !== null) {
        if (check === 'loading') {
          checkPanel = h('div', { className: 'pkl-check-line' }, '正在自检…')
        } else if (check.error !== undefined) {
          checkPanel = h('div', { className: 'pkl-check-line pkl-check-err' }, '自检失败：' + check.error)
        } else {
      ...
      return h('div', { className: 'pkl-view' }, [
        h('div', { className: 'pkl-learn', key: 'learn', style: { paddingBottom: pad + 'px' } }, [
          ...
          checkPanel !== null ? h('div', { className: 'pkl-check', key: 'check' }, checkPanel) : null,
