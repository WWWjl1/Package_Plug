  ...
  const methods = {
    ...
    async load() {
      ...
      return {
        ...
        time: await readJson('progress/time.json', { version: 1, totalSeconds: 0, days: {}, points: {} }),
      }
    },
    ...
    async 'export-progress'() {
      ...
      const time = await readJson('progress/time.json', { version: 1, totalSeconds: 0, days: {}, points: {} })
      return {
    ...
    async 'import-progress'(args) {
      ...
      try {
        ...
        for (const name of ['state.json', 'quiz.json', 'ui.json', 'time.json']) {
      ...
      // 学习时长一起搬（没有这个字段的旧导出件也能正常导入）
      if (bundle.time && typeof bundle.time === 'object' && typeof bundle.time.days === 'object' && bundle.time.days !== null) {
        await writeJson('progress/time.json', {
          version: 1,
          totalSeconds: Number.isInteger(bundle.time.totalSeconds) ? bundle.time.totalSeconds : 0,
          days: bundle.time.days,
          points: (bundle.time.points && typeof bundle.time.points === 'object') ? bundle.time.points : {},
          updatedAt: at,
        })
      }
      return {
    ...
    /**
     * E4 课程导出为 Markdown：给打印、离线复习、或者喂给别的工具（Anki 之类）用。
     *
     * 默认把 SVG 直接嵌成 data URI —— 导出来的**一个文件就是完整的**，
     * 不用连着 assets/ 一起拷、换台机器打开图也还在。
     * 传 { inlineImages: false } 就退回相对路径写法（文件小得多，适合在仓库里看）。
     */
    async 'export-markdown'(args) {
      const inlineImages = !(args && args.inlineImages === false)
      ...
      for (const track of tracks) {
        ...
        for (const proc of list) {
          ...
          courses[proc.id] = course
          const points = Array.isArray(course.points) ? course.points : []
          for (const p of points) where[p.id] = { proc, course, point: p }
        }
      }

      const stats = { modules: 0, points: 0, questions: 0, figures: 0, relatedLinks: 0 }
      const out = []
      const toc = []
      const svgCache = {}
      // 顺手把学习状态也印进去：导出的这份可以当「打印出来打勾」的复习清单
      ...
      const STATUS_LABEL = { unseen: '未学', learning: '在学', mastered: '已掌握' }

      const svgAsDataUri = async (name) => {
        if (svgCache[name] !== undefined) return svgCache[name]
        let uri = ''
        try {
          const text = await readText('assets/' + name)
      ...
      out.push('# 芯片封装学习 · 课程全集')
      out.push('')
      out.push('> 导出时间：' + nowIso())
      out.push('> 内容：三条主线 · ' + String(tracks.length) + ' 套体系；每个细分点都带可计算的数字、')
      out.push('> 「为什么不能反过来做」的反向论证、失效链案例，以及小测题与解析。')
      out.push('')
      ...
      for (const track of tracks) {
        ...
        const trackLines = []
        trackLines.push('')
        trackLines.push('<a id="track-' + track.id + '"></a>')
        trackLines.push('')
        trackLines.push('# ' + esc(track.title || track.id))
        toc.push('- **[' + (track.title || track.id) + '](#track-' + track.id + ')**')
        if (typeof track.hint === 'string' && track.hint !== '') {
          trackLines.push('')
          trackLines.push('> ' + track.hint)
        }
        for (const proc of procs) {
          if (!proc || proc.status !== 'ready') continue
          const course = courses[proc.id]
          if (course === undefined) continue
          stats.modules += 1
          trackLines.push('')
          trackLines.push('<a id="mod-' + proc.id + '"></a>')
          trackLines.push('')
          trackLines.push('## ' + esc(course.title || proc.title))
          if (typeof course.subtitle === 'string' && course.subtitle !== '') {
            trackLines.push('')
            trackLines.push('*' + course.subtitle + '*')
          }
          toc.push('  - [' + (course.title || proc.title) + '](#mod-' + proc.id + ')')
          const points = Array.isArray(course.points) ? course.points : []
          for (const p of points) {
            stats.points += 1
            trackLines.push('')
            trackLines.push('<a id="' + p.id + '"></a>')
            trackLines.push('')
            trackLines.push('### ' + esc(p.title))
            trackLines.push('')
            const meta = []
            if (typeof p.estMinutes === 'number') meta.push('约 ' + String(p.estMinutes) + ' 分钟')
            const st = (marks[p.id] && marks[p.id].status) || 'unseen'
            meta.push(STATUS_LABEL[st] || '未学')
            meta.push('`' + p.id + '`')
            trackLines.push('*' + meta.join(' · ') + '*')
            toc.push('    - [' + (p.title || p.id) + '](#' + p.id + ')')
            ...
            for (const block of body) {
              ...
              if (block.type === 'p') {
                trackLines.push('')
                trackLines.push(String(block.text || ''))
              } else if (block.type === 'h') {
                trackLines.push('')
                trackLines.push('#### ' + esc(block.text))
              } else if (block.type === 'list') {
                trackLines.push('')
                for (const item of (Array.isArray(block.items) ? block.items : [])) {
                  trackLines.push('- ' + String(item === undefined || item === null ? '' : item))
                }
              } else if (block.type === 'table') {
                const head = Array.isArray(block.head) ? block.head : []
                const rowList = Array.isArray(block.rows) ? block.rows : []
                trackLines.push('')
                trackLines.push('| ' + head.map(esc).join(' | ') + ' |')
                trackLines.push('|' + head.map(() => ' --- ').join('|') + '|')
                for (const row of rowList) {
                  const cells = Array.isArray(row) ? row : []
                  trackLines.push('| ' + cells.map(esc).join(' | ') + ' |')
                }
              } else if (block.type === 'figure') {
                const name = typeof block.svg === 'string' ? block.svg : ''
                const caption = typeof block.caption === 'string' ? block.caption : name
                if (name === '') continue
                stats.figures += 1
                const src = inlineImages ? await svgAsDataUri(name) : 'assets/' + name
                trackLines.push('')
                trackLines.push('![' + esc(caption).replace(/]/g, '') + '](' + src + ')')
                if (caption !== '') trackLines.push('')
                if (caption !== '') trackLines.push('*图：' + caption + '*')
              } else if (block.type === 'callout') {
                trackLines.push('')
                const label = block.tone === 'warn' ? '**注意**' : '**重点**'
                trackLines.push('> ' + label + '：' + String(block.text || '').replace(/\r?\n/g, ' '))
              }
            }

            // 相关知识点：导出里也要看得见「这个概念在别的模块也讲过」
            const related = Array.isArray(p.related) ? p.related : []
            const relLines = []
            for (const rid of related) {
              const t = where[rid]
              if (!t) continue
              stats.relatedLinks += 1
              relLines.push('[' + (t.course.title || t.proc.title) + ' · ' + t.point.title + '](#' + rid + ')')
            }
            if (relLines.length > 0) {
              trackLines.push('')
              trackLines.push('**相关知识点**：' + relLines.join(' ｜ '))
            }
            ...
            if (quiz.length > 0) {
              trackLines.push('')
              trackLines.push('**小测（' + String(quiz.length) + ' 题）**')
              trackLines.push('')
              for (let qi = 0; qi < quiz.length; qi += 1) {
                const q = quiz[qi]
                stats.questions += 1
                trackLines.push(String(qi + 1) + '. ' + esc(q.q))
                const options = Array.isArray(q.options) ? q.options : []
                for (let oi = 0; oi < options.length; oi += 1) {
                  trackLines.push('   ' + (oi === q.answer ? '- **[√]** ' : '- [ ] ') + esc(options[oi]))
                }
                if (typeof q.explain === 'string' && q.explain !== '') {
                  trackLines.push('   - 解析：' + q.explain.replace(/\r?\n/g, ' '))
                }
              }
            }
          }
        }
        bodies.push(trackLines)
      }

      // 目录放最前面（正文已经生成完，这里才攒得齐三级目录）
      out.push('## 目录')
      out.push('')
      for (const line of toc) out.push(line)
      out.push('')
      out.push('每一节的锚点就是知识点 id（例如 `#' + (Object.keys(where)[0] || 'wt.why') + '`），')
      out.push('所以导出文件里可以直接用 `文件.md#wt.grind` 这样的链接从别处跳进来。')
      out.push('')
      ...
      return {
        ok: true,
        markdown,
        stats: Object.assign({}, stats, { chars: markdown.length, inlineImages }),
      }
    },
     ...
     * E3 学习时长：收到一次心跳就累加。
     *
     * 只负责「把客户端认定有效的那段时间记下来」——判定规则在客户端
     * （界面在前台、且最近有操作），这里再兜一道：
     * 单次最多认 300 秒，避免某次异常心跳把一天的时间冲爆。
     */
    async 'record-time'(args) {
      const secs = args && args.seconds
      if (!Number.isInteger(secs) || secs <= 0) return { ok: false, reason: 'bad-seconds' }
      const capped = Math.min(secs, 300)
      const cur = await readJson('progress/time.json', { version: 1, totalSeconds: 0, days: {}, points: {} })
      const days = (cur && typeof cur.days === 'object' && cur.days !== null) ? Object.assign({}, cur.days) : {}
      const points = (cur && typeof cur.points === 'object' && cur.points !== null) ? Object.assign({}, cur.points) : {}
      const total = (cur && Number.isInteger(cur.totalSeconds)) ? cur.totalSeconds : 0

      // 按「本地日期」分桶：用 UTC 会在北京时间凌晨 8 点前把昨天算成今天
      const now = new Date()
      const key = now.getFullYear() + '-'
        + String(now.getMonth() + 1).padStart(2, '0') + '-'
        + String(now.getDate()).padStart(2, '0')
      days[key] = (Number.isInteger(days[key]) ? days[key] : 0) + capped
      const pid = args && args.pointId
      if (typeof pid === 'string' && pid !== '') {
        points[pid] = (Number.isInteger(points[pid]) ? points[pid] : 0) + capped
      }
      ...
      if (keys.length > 400) {
        keptDays = {}
        for (const k of keys.slice(-400)) keptDays[k] = days[k]
      }
      await writeJson('progress/time.json', {
        version: 1,
        totalSeconds: total + capped,
        days: keptDays,
        points,
        updatedAt: nowIso(),
      })
      return { ok: true, todaySeconds: days[key], totalSeconds: total + capped }
    },

    /** E3：只把时长数据取出来（比 load 轻得多，界面上刷新统计用它） */
    async 'time-summary'() {
      const time = await readJson('progress/time.json', { version: 1, totalSeconds: 0, days: {}, points: {} })
      return { ok: true, time, now: nowIso() }
    },

    /**
     * E5 设置 / 关于：把「插件装在哪、数据在哪、现在什么状态」摊开给人看。
     * 这些东西平时全藏在文件系统里，出问题时最需要的恰恰是它们。
     */
    async 'about'() {
      // 版本号从**插件包目录**读，不是从数据目录读：
      // 一旦用 config.root 把数据目录指到别处（同步盘之类），那儿没有 package.json，
      // 版本就会是空的。版本描述的是插件，不该跟着数据目录走。
      let version = ''
      try {
      ...
      const files = []
      for (const name of ['state.json', 'quiz.json', 'ui.json', 'session.json', 'time.json']) {
        let bytes = -1
        try {
          bytes = (await readFile(abs('progress/' + name), 'utf8')).length
        } catch (error) {
          bytes = -1
        }
      ...
      let backups = 0
      try {
        backups = (await readdir(abs('progress/backups'))).length
      } catch (error) {
        backups = 0
      }
      const state = await readJson('progress/state.json', { points: {} })
      const marks = (state && state.points) || {}
      let learning = 0
      let mastered = 0
      for (const id of Object.keys(marks)) {
        if (marks[id] && marks[id].status === 'learning') learning += 1
        if (marks[id] && marks[id].status === 'mastered') mastered += 1
      }
      const quiz = await readJson('progress/quiz.json', { wrong: {}, attempts: [] })
      const time = await readJson('progress/time.json', { totalSeconds: 0, days: {} })
      return {
        version,
        root,
        defaultRoot: PACKAGE_ROOT,
        // 数据目录到底是不是被 config.root 改过 —— 界面上要能一眼看出来
        overridden: root !== PACKAGE_ROOT,
        ...
        counts: {
          ...
          learning,
          mastered,
          wrongPoints: Object.keys((quiz && quiz.wrong) || {}).length,
          attempts: Array.isArray(quiz && quiz.attempts) ? quiz.attempts.length : 0,
          studyDays: Object.keys((time && time.days) || {}).length,
          totalSeconds: (time && Number.isInteger(time.totalSeconds)) ? time.totalSeconds : 0,
        },
      }
    },
    ...
    /**
     * E5 清空学习进度。
     * 和导入一样是**不可逆**操作，所以：必须显式传 confirm:'RESET'，
     * 而且先把现有进度备份好；连备份都写不成功就拒绝执行。
     * 只清「学过的痕迹」（状态 / 错题 / 时长），不动会话与界面偏好 —— 那两个清了没有意义。
     */
    async 'reset-progress'(args) {
      if (!args || args.confirm !== 'RESET') return { ok: false, reason: 'need-confirm' }
      const stamp = nowIso().replace(/[:.]/g, '-')
      const backupRel = 'progress/backups/' + stamp + '-before-reset'
      try {
        await mkdir(abs(backupRel), { recursive: true })
        for (const name of ['state.json', 'quiz.json', 'ui.json', 'time.json']) {
          const cur = await readJson('progress/' + name, null)
      ...
      const at = nowIso()
      await writeJson('progress/state.json', { version: 1, updatedAt: at, points: {} })
      await writeJson('progress/quiz.json', { version: 1, attempts: [], wrong: {} })
      await writeJson('progress/time.json', { version: 1, totalSeconds: 0, days: {}, points: {}, updatedAt: at })
      return { ok: true, backupDir: backupRel }
    },
